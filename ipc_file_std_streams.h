...
CPPRS_COMMON_SP decltype(auto) get_file_perms(const std::filesystem::path &p){
	return std::filesystem::status(p).permissions();
}

CPPRS_COMMON_SP decltype(auto) is_file_perms_equal_to_all(const std::filesystem::path &p){
	return get_file_perms(p) == std::filesystem::all;
}

CPPRS_COMMON_SP void set_file_perms_to_readonly(const std::filesystem::path &p){
...
CPPRS_COMMON_SP void set_file_perms_to_all(const std::filesystem::path &p){
	std::filesystem::permissions(p, std::filesystem::all);
}


CPPRS_COMMON_SP decltype(auto) prepare_for_ipc_file_std_streams(void){
...
//}
//
//CPPRS_COMMON_SP void ipc_finish_writing_to_file(const std::filesystem::path &p){
//	//
